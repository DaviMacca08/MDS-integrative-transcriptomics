...
pca_plot_group_be <- ggplot(pca_df, aes(PC1, PC2, color = group)) +
  ...
  ggtitle(paste0("PCA of GSE58831 Samples - SVA (Buja and Eyuboglu Method, SV = ", n_sv_be, ")")) +
  theme_bw()

save_plot(pca_plot_group_be, filename = "pca_plot_SVA_be.png", dir = paths$gse_58831_sva)
...
pca_plot_group_leek <- ggplot(pca_df, aes(PC1, PC2, color = group)) +
  ...
  scale_color_manual(values = c("#CE2915", "#0096FF")) +
  scale_shape_manual(values = c(`FALSE` = 16, `TRUE` = 4)) +
  ggtitle(paste0("PCA of GSE58831 Samples - SVA (Leek Method SV = ", n_sv_leek, ")")) +
  theme_bw()

save_plot(pca_plot_group_leek, filename = "pca_plot_SVA_leek.png", dir = paths$gse_58831_sva)
...
save_csv(deg_table_sva_be, filename = "DEGs_SVA_BE_GSE58831.csv", dir = paths$tables_sva)
save_csv(deg_table_sva_leek, filename = "DEGs_SVA_LEEK_GSE58831.csv", dir = paths$tables_sva)
...
save_session_info(filename = "sessionInfo_DEG_SVA_GSE58831.txt", dir = paths$logs_sva, label = "Differential expression analysis - SVA Correction - GSE58831")
...
message("=================================================")
message("[PIPELINE] Differential expression analysis, using SVA correction, completed successfully for GSE58831 (MDS vs Healthy Controls).")
message("=================================================")
